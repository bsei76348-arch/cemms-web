...
export default function StaffSidebar({ userName = 'Staff' }: StaffSidebarProps) {
  const pathname = usePathname();
  const router = useRouter();

  const handleLogout = async () => {
    try {
      // Try real Firebase signOut first (if using real auth)
      await signOut(auth);
    } catch (error) {
      // If using mock auth, signOut will fail; ignore error
      console.log('Mock logout (no real Firebase session)');
    } finally {
  ...
  const navItems = [
    { href: '/staff', icon: <LayoutDashboard className="w-5 h-5" />, label: 'Dashboard' },
    { href: '/staff/records', icon: <Archive className="w-5 h-5" />, label: 'Records' },
    { href: '/staff/live-map', icon: <Map className="w-5 h-5" />, label: 'Live Map' },
    { href: '/staff/live-stats', icon: <TrendingUp className="w-5 h-5" />, label: 'Live Stats' },
    { href: '/staff/reports', icon: <FileText className="w-5 h-5" />, label: 'Reports' },
    { href: '/staff/settings', icon: <Settings className="w-5 h-5" />, label: 'Settings' },
  ];
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '36px' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#6B7280' }}>Staff Portal</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#3B7A6A' }}>MENRO Staff</div>
        </div>
      </div>
