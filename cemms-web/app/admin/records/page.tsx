// app/admin/records/page.tsx – Actions as text buttons
'use client';
...
import { collection, getDocs, onSnapshot, doc, addDoc, updateDoc, deleteDoc, query, orderBy, limit } from 'firebase/firestore';
...
export default function RecordsPage() {
  ...
  const [currentRecord, setCurrentRecord] = useState<CEMMSRecord | null>(null);
  ...
  const openEditModal = (record: CEMMSRecord) => {
    setModalMode('edit');
  ...
  const handleDelete = async (record: CEMMSRecord) => {
    if (!confirm(`Delete record from ${record.barangay} (${record.amount} kg)?`)) return;
    try {
      const dbRef = record.collectionName === 'emissions' ? webCemmsDb : mobileDb;
      await deleteDoc(doc(dbRef, record.collectionName, record.docId));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(amountNum) || amountNum <= 0) {
      addToast('error', 'Amount must be a positive number');
      return;
    }
    if (!formData.barangay) {
      addToast('error', 'Please select a barangay');
      return;
    }
    setSubmitting(true);
    try {
      if (modalMode === 'add') {
        const newDoc = {
          ...
          amount: amountNum,
          carbonAmount: amountNum,
          type: 'Web Input',
          createdAt: new Date(formData.date),
          source: 'Web Admin'
        };
        await addDoc(collection(webCemmsDb, 'emissions'), newDoc);
        addToast('success', 'Record added to Web database');
      } 
      else if (modalMode === 'edit' && currentRecord) {
        const dbRef = currentRecord.collectionName === 'emissions' ? webCemmsDb : mobileDb;
        const updateData: any = {
          barangay: formData.barangay,
          amount: amountNum,
          updatedAt: new Date()
        };
        if (currentRecord.collectionName === 'emissions') {
          updateData.carbonAmount = amountNum;
        } else {
          updateData.dailyCarbon = amountNum;
        }
        await updateDoc(doc(dbRef, currentRecord.collectionName, currentRecord.docId), updateData);
        addToast('success', 'Record updated');
      }
      setModalOpen(false);
    } catch (err: any) {
      console.error(err);
      addToast('error', `Failed to ${modalMode}: ` + err.message);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div style={{ display: 'flex', background: '#F8FDF9', minHeight: '100vh' }}>
      ...
      <div className="main-content">
        ...
        <div className="header-card">
          <div>
            <h1>Records</h1>
            <p>Browse and manage emission records from Web and Mobile apps.</p>
          </div>
          <div className="header-actions">
            <button className="icon-btn primary" onClick={openAddModal}><Plus size={16} /> Add Record</button>
                  ...
                  {filteredWeb.map(r => (
                    <tr key={r.id}>
                      ...
                      <td className="actions-cell">
                        <button className="action-btn edit" onClick={() => openEditModal(r)}>Edit</button>
                        <button className="action-btn delete" onClick={() => handleDelete(r)}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="modal-overlay" onClick={() => setModalOpen(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{modalMode === 'add' ? 'Add New Record' : 'Edit Record'}</h3>
              <button className="close-btn" onClick={() => setModalOpen(false)}><X size={18} /></button>
            </div>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <div className="static-type">
                  {modalMode === 'add' ? 'Web Input' : (currentRecord?.type || '—')}
                </div>
              </div>
              <div className="form-group">
                <label>Date</label>
      ...
      <style jsx>{`
        ...
        .icon-btn.primary { background: #E9F5DB; color: #3D5C1A; border: 1px solid #D4E8B3; }
        .icon-btn.primary:hover { background: #D4E8B3; transform: translateY(-1px); }
        .icon-btn:hover:not(.primary) { background: #E6F7F3; transform: translateY(-1px); }
        .live-badge {
          display: flex; align-items: center; gap: 8px;
